...
// @TODO: Translation point-1 point-2
const data = {
  invoices: [
    {
      id: 4987,
...
mock.onGet("/apps/invoice/invoices").reply((config) => {
  ...
  const filteredData = data.invoices.filter((invoice) => {
    if (dates.length) {
      ...
      range.filter((date) => {
        console.log(status.toLowerCase() ,invoice.invoiceStatus.toLowerCase())
      ...
      if (filtered.length && filtered.includes(invoice.id)) {
        return (
          (invoice.companyEmail.toLowerCase().includes(queryLowered) ||
            ...
            invoice.payer.toLowerCase().includes(queryLowered) ||
